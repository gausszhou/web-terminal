...
export class Frame {
  ...
  constructor(type: FrameType, payload: Uint8Array) {
    this.type = type;
...
export class FrameCodec {
  static encode(type: FrameType, data: string | number | Uint8Array): Uint8Array {
    ...
    const frame = new Frame(type, playload);
  ...
  static decode(data: Uint8Array): Frame {
    // 检查数据长度是否足够
    if (data.length < Frame.HeaderSize) {
      throw new Error(`Data too short: expected at least ${Frame.HeaderSize} bytes, got ${data.length}`);
    }
    ...
    // 检查payload长度是否有效
    if (data.length < Frame.HeaderSize + payloadLength) {
      throw new Error(`Incomplete data: expected ${Frame.HeaderSize + payloadLength} bytes, got ${data.length}`);
    }
  ...
  static buffer2number(data: Uint8Array): number {
    ...
    if (data.length < 8) {
      throw new Error(`Data too short for number conversion: expected 8 bytes, got ${data.length}`);
    }
